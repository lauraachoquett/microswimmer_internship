...
class MicroSwimmer(gym.Env):
    ...
    def state(self, tree, path):
        ...
        if self.n_lookahead > 0:
            ...
            lookahead_local = [
                coordinate_in_path_ref(p_cp, self.dir_path, p) for p in lookahead
            ]
            if self.velocity_bool:
                return np.concatenate(
                    (
                        s.reshape(1, 2),
                        v_local_path.reshape(1, 2),
                        np.array(lookahead_local),
                    ),
                    axis=0,
                )
            else:
                return np.concatenate(
                    (s.reshape(1, 2).reshape(1, 2), np.array(lookahead_local)), axis=0
                )
        if self.velocity_bool:
    ...
    def reset(self, tree=None, path=None):
        self.x = self.x_0
