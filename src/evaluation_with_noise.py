...
def evaluate_after_training(
    ...
    for agent_name in agent_files:
        ...
        for path_to_config in tqdm(list_config_paths, desc="Processing configs"):
            ...
            config_eval['nb_points_path'] = len(path)
...
def obstacle_and_path(
    ...
    B = config_par_path["B"]
    c = config_par_path["c"]
    h = config_par_path["heuristic_weight"]
    ...
    if path_to_config_path is not None:
        ...
        start_point = (
            start_point[0] * physical_width,
            start_point[1] * physical_height,
            start_point[2] * physical_depth,
        )
        goal_point = (goal_point[0] * physical_width, goal_point[1] * physical_height,goal_point[2]*physical_depth)
        ...
        if path_method == "astar":
            ...
            v0,vx,vy,vz,save_path_phi, save_path_flow = compute_v(x_phys, y_phys, z_phys,vx_phys,vy_phys,vz_phys, B, grid_size, ratio, sdf_func, c)
            if type == "free":
                v0 = np.ones_like(v0)
                vx = None
                vy = None
...
def sdf_func_ez(domain_size, ratio):
    x, y, z, N, h, sdf = load_sdf_from_csv(domain_size)
...
def create_all_path(config_par_path, nb_points, failure_cases=False):
    ...
    if compute_goal_points:
        ...
        goal_points = create_list_of_goal_point(
            30000, config_par_path["start_point"], ratio
        )
...
def create_list_of_goal_point(n, start_point, ratio):
    point_list = np.random.rand(n, 3)
    sdf_function = sdf_func_ez((1, 1,1), ratio)
    goal_points = [
        point
