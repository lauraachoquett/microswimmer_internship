...
def fmm_path_indices(start, goal, sdf, min_distance=0.1):
    ...
    if np.any(safe_regions):
        ...
        if max_sdf > min_distance:
            ...
            speed[safe_regions] = 0.2 + 0.8 * (normalized_sdf[safe_regions] - min_distance) / (max_sdf - min_distance)
    ...
    if len(simplified_path) <= 1 or not (abs(simplified_path[-1][0] - goal[0]) <= 1 and abs(simplified_path[-1][1] - goal[1]) <= 1):
