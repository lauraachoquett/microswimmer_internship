...
class Actor(nn.Module):
	...
	def forward(self, state):
		...
		a = F.relu(self.l2(a))
		a = self.l3(a)
		a = F.normalize(a, p=2, dim=-1)  # norme L2 = 1
		return a
...
class TD3(object):
	def __init__(
		...
		self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=3e-4,weight_decay=0.0001)

		self.critic = Critic(state_dim, action_dim).to(device)
		self.critic_target = copy.deepcopy(self.critic)
		self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=3e-4,weight_decay=0.0001)
